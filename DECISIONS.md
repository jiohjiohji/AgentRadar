...
2026-04-01 | MVP ranking proxy: status + tag overlap + composite score (stars deferred) | Phase 1 | github_stars is not in the schema (schema is 12 fields, Phase 0 decision). For MVP, composite score is a reasonable adoption proxy. Stars to be added when the daily crawler enriches profiles. | Blocking on stars: would require schema change + re-crawl before shipping the plugin
